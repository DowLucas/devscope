...
export function alertsRoutes(sql: SQL) {
  ...
  app.get("/rules", async (c) => {
    ...
    return c.json(await getAlertRules(sql, orgId));
  });
  ...
  app.put("/rules/:id", zValidator("json", alertRuleUpdateSchema), async (c) => {
    ...
    const body = c.req.valid("json");
    const orgId = c.get("orgId" as never) as string | undefined;
    await updateAlertRule(sql, id, body, orgId);
  ...
  app.delete("/rules/:id", async (c) => {
    const id = c.req.param("id");
    const orgId = c.get("orgId" as never) as string | undefined;
    await deleteAlertRule(sql, id, orgId);
    return c.json({ ok: true });
  });
  ...
  app.get("/recent", async (c) => {
    ...
    const orgId = c.get("orgId" as never) as string | undefined;
    return c.json(await getRecentAlerts(sql, limit, orgId));
  });
