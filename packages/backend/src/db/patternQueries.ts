...
export async function getPatterns(
  sql: SQL,
  opts?: { effectiveness?: string; category?: string; limit?: number; minOccurrences?: number }
): Promise<SessionPattern[]> {
  ...
  if (opts?.effectiveness && opts?.category) {
    return (await sql`
      SELECT * FROM session_patterns WHERE effectiveness = ${opts.effectiveness} AND category = ${opts.category} AND occurrence_count >= ${minOcc}
      ORDER BY occurrence_count DESC, avg_success_rate DESC LIMIT ${limit}`) as SessionPattern[];
  }
  if (opts?.effectiveness) {
    return (await sql`
      SELECT * FROM session_patterns WHERE effectiveness = ${opts.effectiveness} AND occurrence_count >= ${minOcc}
      ORDER BY occurrence_count DESC, avg_success_rate DESC LIMIT ${limit}`) as SessionPattern[];
  }
  if (opts?.category) {
    return (await sql`
      SELECT * FROM session_patterns WHERE category = ${opts.category} AND occurrence_count >= ${minOcc}
      ORDER BY occurrence_count DESC, avg_success_rate DESC LIMIT ${limit}`) as SessionPattern[];
  }
  return (await sql`
    SELECT * FROM session_patterns WHERE occurrence_count >= ${minOcc}
    ORDER BY occurrence_count DESC, avg_success_rate DESC LIMIT ${limit}`) as SessionPattern[];
}

export async function getPatternById(sql: SQL, id: string): Promise<SessionPattern | null> {
  const [row] = await sql`SELECT * FROM session_patterns WHERE id = ${id}`;
  return (row as SessionPattern) ?? null;
}

export async function getPatternStats(sql: SQL, days: number = 30): Promise<{
  total_patterns: number; effective_count: number; ineffective_count: number;
  top_patterns: SessionPattern[]; recent_matches: number;
}> {
  const [counts] = await sql`
    SELECT COUNT(*)::INT as total_patterns,
  ...
  const topPatterns = await sql`SELECT * FROM session_patterns WHERE effectiveness = 'effective' ORDER BY occurrence_count DESC LIMIT 5`;
  const [matchCount] = await sql`SELECT COUNT(*)::INT as cnt FROM session_pattern_matches WHERE created_at >= NOW() - make_interval(days => ${days})`;
  return {
...
export async function getSessionPatterns(sql: SQL, sessionId: string): Promise<(SessionPattern & { match_confidence: number })[]> {
  return (await sql`
    SELECT sp.*, spm.match_confidence FROM session_patterns sp
    JOIN session_pattern_matches spm ON sp.id = spm.pattern_id
    WHERE spm.session_id = ${sessionId} ORDER BY spm.match_confidence DESC`) as any[];
}

export async function getDeveloperToolMastery(sql: SQL, developerId: string, weeks: number = 12): Promise<{
  week: string; tool_name: string; successes: number; total: number; success_rate: number;
}[]> {
  return (await sql`
    SELECT date_trunc('week', e.created_at)::DATE as week, e.payload->>'toolName' as tool_name,
    ...
    WHERE s.developer_id = ${developerId} AND e.event_type IN ('tool.complete', 'tool.fail')
      AND e.payload->>'toolName' IS NOT NULL AND e.created_at >= NOW() - make_interval(weeks => ${weeks})
    GROUP BY week, tool_name ORDER BY week ASC, total DESC`) as any[];
}

export async function getDeveloperPatternAdoption(sql: SQL, developerId: string, weeks: number = 12): Promise<{
  week: string; effective_count: number; ineffective_count: number; neutral_count: number;
}[]> {
  return (await sql`
    SELECT date_trunc('week', spm.created_at)::DATE as week,
    ...
    FROM session_pattern_matches spm JOIN session_patterns sp ON spm.pattern_id = sp.id
    JOIN sessions s ON spm.session_id = s.id
    WHERE s.developer_id = ${developerId} AND spm.created_at >= NOW() - make_interval(weeks => ${weeks})
    GROUP BY week ORDER BY week ASC`) as any[];
}

export async function getDeveloperSessionQuality(sql: SQL, developerId: string, weeks: number = 12): Promise<{
  week: string; sessions: number; avg_success_rate: number; avg_tool_calls: number;
}[]> {
  return (await sql`
    SELECT date_trunc('week', s.started_at)::DATE as week,
      COUNT(DISTINCT s.id)::INT as sessions,
      ROUND((SUM(CASE WHEN e.event_type = 'tool.complete' THEN 1 ELSE 0 END)::FLOAT /
        GREATEST(SUM(CASE WHEN e.event_type IN ('tool.complete', 'tool.fail') THEN 1 ELSE 0 END), 1))::NUMERIC, 3)::FLOAT as avg_success_rate,
      ROUND((COUNT(CASE WHEN e.event_type IN ('tool.complete', 'tool.fail') THEN 1 END)::FLOAT /
        GREATEST(COUNT(DISTINCT s.id), 1))::NUMERIC, 1)::FLOAT as avg_tool_calls
    FROM sessions s LEFT JOIN events e ON e.session_id = s.id AND e.event_type IN ('tool.complete', 'tool.fail')
    WHERE s.developer_id = ${developerId} AND s.started_at >= NOW() - make_interval(weeks => ${weeks})
    GROUP BY week ORDER BY week ASC`) as any[];
}
