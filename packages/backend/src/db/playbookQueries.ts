...
export async function updatePlaybook(
  ...
  if (updates.name !== undefined) {
    await sql`UPDATE playbooks SET name = ${updates.name}, updated_at = NOW() WHERE id = ${id}`;
  }
  if (updates.description !== undefined) {
    await sql`UPDATE playbooks SET description = ${updates.description}, updated_at = NOW() WHERE id = ${id}`;
  }
  if (updates.when_to_use !== undefined) {
    await sql`UPDATE playbooks SET when_to_use = ${updates.when_to_use}, updated_at = NOW() WHERE id = ${id}`;
  }
  if (updates.status !== undefined) {
    await sql`UPDATE playbooks SET status = ${updates.status}, updated_at = NOW() WHERE id = ${id}`;
  }
  const [row] = await sql`SELECT * FROM playbooks WHERE id = ${id}`;
  return (row as Playbook) ?? null;
}
