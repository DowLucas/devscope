...
export async function createAntiPatternMatch(
  sql: SQL,
  sessionId: string,
...
export async function getAntiPatternStats(
  ...
  top_anti_patterns: AntiPattern[];
  recent_matches: number;
  by_rule: { detection_rule: string; count: number }[];
}> {
  ...
  return {
    total_anti_patterns: (counts as any)?.total_anti_patterns ?? 0,
    critical_count: (counts as any)?.critical_count ?? 0,
    warning_count: (counts as any)?.warning_count ?? 0,
    top_anti_patterns: topAntiPatterns as AntiPattern[],
    recent_matches: (matchCount as any)?.cnt ?? 0,
    by_rule: byRule as any[],
  };
}
...
export async function getDeveloperAntiPatterns(
  sql: SQL,
  developerId: string,
  weeks: number = 12
): Promise<{
